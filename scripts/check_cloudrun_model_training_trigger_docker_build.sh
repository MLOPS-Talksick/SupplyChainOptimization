...
echo "🔍 Checking if files in 'model_development/model_training_cloud_run_trigger' have changed..."

if git rev-parse HEAD~1 >/dev/null 2>&1; then
  if git diff --quiet HEAD~1 HEAD -- model_development/model_training_cloud_run_trigger; then
  ...
  if git diff --quiet -- model_development/model_training_cloud_run_trigger; then
