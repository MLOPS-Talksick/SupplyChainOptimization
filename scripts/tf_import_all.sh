...
######################################
FORWARDING_RULE_NAME="airflow-http-forwarding-rule"
...
if gcloud functions describe "${FUNCTION_NAME}" --region="${REGION}" --project="${PROJECT_ID}" &>/dev/null; then
    ...
    terraform import google_cloudfunctions2_function.process_data_function "projects/${PROJECT_ID}/locations/${REGION}/functions/${FUNCTION_NAME}"
else
