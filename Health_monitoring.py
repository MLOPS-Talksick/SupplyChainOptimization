...
def verify_token(token: str = Header(None)):
    """Verify API token from the request header"""
...
async def model_health_check(token: str = Depends(verify_token)):
    ...
    send_metrics_to_cloud_monitoring(metrics)
...
def send_metrics_to_cloud_monitoring(metrics):
    """
    Send model health metrics to Google Cloud Monitoring
    """
    try:
        # Create the client
        ...
        # Define metric descriptors if they don't already exist
        metric_descriptors = {
            "rmse": "custom.googleapis.com/ml/model_health/rmse",
            "mape": "custom.googleapis.com/ml/model_health/mape",
            "p_value": "custom.googleapis.com/ml/model_health/p_value",
            "ks_statistic": "custom.googleapis.com/ml/model_health/ks_statistic",
            "sample_size": "custom.googleapis.com/ml/model_health/sample_size"
        }
        ...
        for metric_name, metric_value in metrics.items():
            ...
            series.append({
                "metric": {
                    "type": metric_descriptors[metric_name],
                    "labels": {
                        "model": "supply_chain_forecast"
                    }
                },
                "resource": {
                    "type": "global",
        ...
        # Send all series in one request
        if series:
            client.create_time_series(name=project_name, time_series=series)
            logging.info(f"Successfully sent {len(series)} metrics to Cloud Monitoring")
        else:
            logging.warning("No valid metrics to send to Cloud Monitoring")
            
        return True
    except Exception as e:
        logging.error(f"Failed to send metrics to Cloud Monitoring: {str(e)}")
        return False


def create_monitoring_dashboard():
    """
    Create a Google Cloud Monitoring dashboard for model health metrics
    """
    try:
        client = monitoring.DashboardsServiceClient()
        project_name = f"projects/{PROJECT_ID}"
        
        dashboard = {
            "displayName": "Model Health Monitoring",
            "gridLayout": {
                "widgets": [
                    # RMSE Widget
                    {
                        ...
                        "xyChart": {
                            "dataSets": [{
                                "timeSeriesQuery": {
                                    "timeSeriesFilter": {
                                        "filter": 'metric.type="custom.googleapis.com/ml/model_health/rmse"',
                                        "aggregation": {
                    ...
                    # P-Value Widget
                    {
                        "title": "Data Drift (p-value)",
                        "xyChart": {
                            "dataSets": [{
                                "timeSeriesQuery": {
                                    "timeSeriesFilter": {
                                        "filter": 'metric.type="custom.googleapis.com/ml/model_health/p_value"',
                                        "aggregation": {
                                            "alignmentPeriod": "86400s",
                                            "perSeriesAligner": "ALIGN_MEAN",
                                        }
                    ...
                    # MAPE Widget
                    {
                        ...
                        "xyChart": {
                            "dataSets": [{
                                "timeSeriesQuery": {
                                    "timeSeriesFilter": {
                                        "filter": 'metric.type="custom.googleapis.com/ml/model_health/mape"',
                                        "aggregation": {
                                            "alignmentPeriod": "86400s",
                                            "perSeriesAligner": "ALIGN_MEAN",
                                        }
                    ...
                    # Sample Size Widget
                    {
                        ...
                        "xyChart": {
                            "dataSets": [{
                                "timeSeriesQuery": {
                                    "timeSeriesFilter": {
                                        "filter": 'metric.type="custom.googleapis.com/ml/model_health/sample_size"',
                                        "aggregation": {
                                            "alignmentPeriod": "86400s",
                                            "perSeriesAligner": "ALIGN_MEAN",
                                        }
        ...
        logging.info(f"Created dashboard: {created_dashboard.name}")
...
def create_alert_policy():
    """Create alert policies for model health metrics"""
    try:
        client = monitoring.AlertPolicyServiceClient()
        project_name = f"projects/{PROJECT_ID}"
        
        # RMSE Alert Policy
        rmse_policy = {
            "display_name": "Model Health - RMSE Exceeded Threshold",
            "conditions": [{
                "display_name": "RMSE Threshold Alert",
                "condition_threshold": {
                    "filter": f'metric.type="custom.googleapis.com/ml/model_health/rmse" AND resource.type="global"',
                    "comparison": "COMPARISON_GT",
                    "threshold_value": RMSE_THRESHOLD,
                    "duration": {"seconds": 0},
                    "trigger": {
                        "count": 1
                    },
        ...
        # Create the RMSE alert policy
        created_rmse_policy = client.create_alert_policy(
            name=project_name,
            alert_policy=rmse_policy
        )
        
        logging.info(f"Created RMSE alert policy: {created_rmse_policy.name}")
        
        # P-value alert policy (data drift detection)
        p_value_policy = {
            "display_name": "Model Health - Data Drift Detected",
            "conditions": [{
                "display_name": "P-value Threshold Alert",
                "condition_threshold": {
                    "filter": f'metric.type="custom.googleapis.com/ml/model_health/p_value" AND resource.type="global"',
        ...
        # Create the p-value alert policy
        created_p_value_policy = client.create_alert_policy(
            name=project_name,
            alert_policy=p_value_policy
        )
        
        logging.info(f"Created p-value alert policy: {created_p_value_policy.name}")

        return True
    except Exception as e:
        logging.error(f"Failed to create alert policies: {str(e)}")
        return False
...
# Make sure to initialize the table and monitoring setup
from contextlib import asynccontextmanager

@asynccontextmanager
async def lifespan(app: FastAPI):
    """Lifespan event handler for FastAPI"""
    # Startup: Create database table and initialize monitoring
    ...
    try:
        ...
        if dashboard:
            logging.info("Cloud Monitoring dashboard created successfully")
        
        # Create alert policies
        alerts_created = create_alert_policy()
        if alerts_created:
            logging.info("Cloud Monitoring alert policies created successfully")
    except Exception as e:
    ...
    logging.info("Health check API shutting down")

# And when creating your FastAPI app:
from fastapi import FastAPI

# Create the FastAPI app with lifespan handler
app = FastAPI(lifespan=lifespan)

# Create router and attach to app
router = APIRouter()
